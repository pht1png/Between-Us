...
4. **Grouping** (`computeGroups`): greedy male–female pairing first, then leftovers from the surplus sex join the smallest eligible group they fit (capped by `MAX_GROUP_SIZE`), widening to larger groups only when nothing at the current size is comparable — this spreads leftovers breadth-first instead of hoarding them into whichever group won the first tie. Then any true residual — or an all-one-sex room — falls back to same-sex pairing via the shared `pairBySimilarity` helper. Every group carries `formedVia`, and the UI must keep surfacing non-`primary-pair` groups as such (the "จับคู่พิเศษ" badge); silently passing a fallback off as a normal match is a product bug.
